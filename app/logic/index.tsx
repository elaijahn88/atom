...
export const useAuthLogic = () => {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        setMessage(`✅ Welcome back, ${(docSnap.data() as IUserData).name || "User"}!`);
      }
      setIsLoggedIn(true);
    } catch (err: any) {
      setMsg("Error: " + err.message);
  ...
  return {
    state: {
      email, password, confirmPassword, name, account, age,
      loading, message, isLoginMode, showPassword, showConfirmPassword,
      isLoggedIn, user, currentIndex, videos, showVideoFeed,
      videoRefs, onViewableItemsChanged, viewConfigRef
    },
    setters: {
      setEmail, setPassword, setConfirmPassword, setName,
      setAccount, setAge, setIsLoginMode, setShowPassword,
      setShowConfirmPassword
    },
    handlers: { handleSignIn, handleSignUp, handleLogout },
  };
};
