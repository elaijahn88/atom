...
export default function MyStore({ currentUserEmail }: { currentUserEmail: string }) {
  ...
  // Split data into pages of 6 items
  const pages: Product[][] = [];
        ...
        <View key={pageIndex} style={styles.page}>
          {pageItems.map((item) => (
            <TouchableOpacity
              key={item.id}
              activeOpacity={0.9}
              style={[styles.card, { backgroundColor: isDark ? "#1c1c1e" : "#fff", shadowColor: isDark ? "#000" : "#ddd" }]}
            >
              <Image
                source={{ uri: item.image || "https://xlijah.com/pics/phones/iphone/12.jpg" }}
                style={styles.image}
              />
...
const styles = StyleSheet.create({
  ...
  page: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", paddingHorizontal: 12, marginBottom: 16 },
  card: { width: CARD_WIDTH, borderRadius: 14, padding: 10, marginBottom: 16, elevation: 3, shadowOpacity: 0.2, shadowRadius: 6, shadowOffset: { width: 0, height: 3 } },
