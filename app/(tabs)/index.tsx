...
const menuItems: FoodItem[] = [
  { id: 1, name: "Classic Burger", price: 600000, image: "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=800", ownerName: "Restaurant One", ownerPhone: "+256700000001", ownerLocation: "Kampala" },
  { id: 2, name: "Pepperoni Pizza", price: 1000000, image: "https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?w=800", ownerName: "Pizza Hub", ownerPhone: "+256700000002", ownerLocation: "Ntinda" },
  { id: 3, name: "Grilled Chicken", price: 900000, image: "https://images.unsplash.com/photo-1599486092146-4d4c7e3c7d3e?w=800", ownerName: "Chicken Spot", ownerPhone: "+256700000003", ownerLocation: "Kawempe" },
  { id: 4, name: "African Milk Tea (Chai)", price: 200000, image: "https://images.unsplash.com/photo-1517701604599-bb29b565090c?w=800", ownerName: "Tea Corner", ownerPhone: "+256700000004", ownerLocation: "Mukono" },
];
...
export default function FoodOrderingApp() {
  ...
  const addToCart = async (item: FoodItem) => {
    ...
    if (walletBalance < item.price) return Alert.alert("Insufficient Balance", "You don't have enough money in your wallet.");
    ...
    sendLocalNotification("Added to Cart 🛒", `${item.name} added`);
  };
  ...
  const handleCheckout = async () => {
    ...
    sendLocalNotification("Order Placed!", `You paid UGX ${totalAmount.toLocaleString()}`);
    await notifyAllUsers("New Order", `${username} just ordered UGX ${totalAmount.toLocaleString()}!`);
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.walletText}>Wallet: <Text style={styles.walletAmount}>UGX {walletBalance.toLocaleString()}</Text></Text>
        </View>

        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
          {menuItems.map(item => (
            <TouchableOpacity key={item.id} style={styles.menuItem} activeOpacity={0.92} onPress={() => addToCart(item)}>
              <View style={styles.imageContainer}><Image source={{ uri: item.image }} style={styles.foodImage} /></View>
              <View style={styles.itemInfo}>
                <Text style={styles.itemName}>{item.name}</Text>
                <Text style={styles.itemPrice}>UGX {item.price.toLocaleString()}</Text>
              </View>
              <TouchableOpacity style={styles.addButton} onPress={() => addToCart(item)}><Text style={styles.addButtonText}>+</Text></TouchableOpacity>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {cart.length > 0 && (
          <Animated.View style={[styles.cartContainer, { transform: [{ scale: cartScale }] }]}>
            <Text style={styles.cartText}>🛒 {cart.length} items • UGX {totalCartAmount.toLocaleString()}</Text>
            <TouchableOpacity style={styles.checkoutButton} onPress={handleCheckout}><Text style={styles.checkoutText}>Checkout</Text></TouchableOpacity>
          </Animated.View>
        )}
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.loginContainer}>
        ...
        <TouchableOpacity style={styles.updateButton} onPress={() => setActiveScreen("foodMenu")}>
...
const styles = StyleSheet.create({
  ...
  updateButton: { backgroundColor: "#10B981", paddingVertical: 18, borderRadius: 20, alignItems: "center" },
